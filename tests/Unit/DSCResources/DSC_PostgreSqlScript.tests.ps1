        ...
        Describe "$moduleResourceName\Set-TargetResource" -Tag 'Set'{
            ...
            Context 'When Set-TargetResource fails' {
                It 'Should throw when psql is not found' {
                    $invalidParams = $scriptParams.Clone()
                    $invalidParams.PsqlLocation = 'Z:\does-not-exist.exe'
                    $errorMessage = "The term 'Z:\does-not-exist.exe' is not recognized as the name of a cmdlet, function, script file, or operable program. Check the spelling of the name, or if a path was included, verify that the path is correct and try again."
